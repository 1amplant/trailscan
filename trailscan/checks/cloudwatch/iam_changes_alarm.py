...
class IAMChangesAlarm(BaseCheck):
    check_id = "cloudwatch.iam_changes_alarm"
    control_id = "CC6.1"
    title = "CloudWatch alarm exists for IAM policy changes"
    remediation = (
        "Create a CloudWatch metric filter and alarm for IAM policy changes: "
        "1. CloudWatch → Log groups → select CloudTrail log group → Metric filters → Create. "
        "2. Filter pattern: { ($.eventName = DeleteGroupPolicy) || ($.eventName = DeleteRolePolicy) || "
    ...
    def run(self, session: boto3.Session, account_id: str) -> list[Finding]:
        ...
        resource_arn = f"arn:aws:cloudwatch::{account_id}:alarm/iam-changes"
        ...
        for lg_name, region in log_groups:
            ...
            for f in get_metric_filters_for_log_group(logs_client, lg_name):
                if not filter_matches_keywords(f.get("filterPattern", ""), _IAM_KEYWORDS):
                    continue
